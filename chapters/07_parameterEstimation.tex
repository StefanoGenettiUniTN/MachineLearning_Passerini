...
\section{Maximum-likelyhood estimation}
    ...
    \subsection{Maximizing the log-likelyhood}
    	...
    	\begin{itemize}
    	    \item the log of the likelyhood is
    	    $$\sum_{j=1}^n -\frac{1}{2} (x_j - \mu)^t \Sigma^{-1}(x_j - \mu) -\frac{1}{2}log(2\pi)^d|\Sigma|$$
    	    \item the maximum likelyhood estimates are the vector of the sample means
    	    $$\mu = \frac{1}{n}\sum_{j=1}^n x_j$$
    	    and the covariances matrix
    	    $$\Sigma = \frac{1}{n} \sum_{j=1}^n (x_j - \mu)(x_j - \mu)^t$$
    	\end{itemize}
    	
    	In the general case of a Gaussian distribution the maximum likelyhood parameters are simply their empirical estimates over the samples.\\
...
\section{Conjugate priors}
    ...
    Given the likelyhood function $p(x|\Theta)$ and the prior distribution $p(\Theta)$, then
    $$p(\Theta)$$
