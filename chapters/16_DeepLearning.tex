...
\textbf{Remark:} the neuron in the input layer are not exactly neuron. Indeed,
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth, node distance=1.5cm]
		\centering
		% Input layer
		\node (and) [circle, draw, minimum size=0.8cm] {AND};

		% Hidden layer (OR)
		\node (or1) [circle, draw, minimum size=0.8cm, below=of and] {OR}; \node (or2)
		[circle, draw, minimum size=0.8cm, left=of or1] {OR}; \node (or3) [circle,
		draw, minimum size=0.8cm, right=of or1] {OR};

		% Output layer
		\node (output1) [circle, draw, minimum size=0.8cm, below=of or2] {Output}; \node
		(output2) [circle, draw, minimum size=0.8cm, below=of or3] {Output}; \node (output3)
		[circle, draw, minimum size=0.8cm, below=of or1] {Output};

		% Connect nodes
		\draw[->] (and) -- (or1); \draw[->] (and) -- (or2); \draw[->] (and) -- (or3);
		\draw[->] (or1) -- (output1); \draw[->] (or1) -- (output2); \draw[->] (or1)
		-- (output3); \draw[->] (or2) -- (output1); \draw[->] (or2) -- (output2);
		\draw[->] (or2) -- (output3); \draw[->] (or3) -- (output1); \draw[->] (or3)
		-- (output2); \draw[->] (or3) -- (output3);
	\end{tikzpicture}
	\caption{Neural network with input layer (AND), hidden layer (OR) with 3 neurons
	and output layer with 3 neurons.}
	\label{fig:deep_boolean_fun}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth, node distance=1.5cm]
		\centering
		% Output layer
		\node (input1) [circle, draw, minimum size=0.8cm] {$j$}; \node (output2) [circle,
		draw, minimum size=0.8cm, right = of input1] {$l$};

		% Connect nodes
		\draw[->] (input1) -- (output2)node[midway, above, font=\small]{$w_{lj}$};
	\end{tikzpicture}
	\caption{$w_{lj}= w_{lj}- \eta \frac{\partial E(W)}{\partial w_{lj}}$}
	\label{fig:MLP_stochastic_gradient_descent}
\end{figure}

The difficult thing is to compute the partial derivative because the weights could
be really far from the output layer where we actually compute the error. The
solution of this problem is faced with \textit{backpropagation}. The idea is to
use the chain rule for derivation. Each weight of the network is updated according
...
\begin{equation}
	\frac{\partial E(W)}{\partial w_{li}}= \frac{\partial E(W)}{\partial a_{l}}\frac{\partial
	a_{l}}{w_{lj}}= \delta_{l}\phi_{j}\label{eq:backpropagation}
\end{equation}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{
		images/16_DeepLearning_backpropagation.png
	}
	\caption{The idea of backpropagation is to use the chain rule for derivation.}
	\label{fig:backpropagation}
\end{figure}

\textbf{Remark:} the big circle in Figure \ref{fig:backpropagation} is the node $l$
and $\phi_{l}$ is the output of node $l$.
\newline

Looking inside node $l$ we find the weighted sum of the inputs of the node including
...
Overall, this is a modular architecture: we do not care about what $F_{j}$ does
internally.
\newline
...
\textbf{Remark:} training kernel machines requires solving quadratic optimization
...
Perhaps, one of the main reasons why this architecture started to be trainable is
...
There is an alternative form of regularization that can be used which is \textit{1-norm
...
\textbf{Remark:} studying autoencoders we can highlight a crucial difference with
...
A combination of filters and convolution layers is repeated along the network in
...
Inside a cell, the overall state is a combination of the part of the previous state
...
We need one more technical detail to make the Transformers easier to understand:
...
The Encored is on the left and the Decoder is on the right. Both Encoder and Decoder
...
The righthand picture describes how this attention-mechanism can be parallelized
...
But there are more complex graphs, without a fixed form, with a variable size of
...
It’s very difficult to perform CNN on graphs because of the arbitrary size of
...
Locality information can be achieved by using a \textit{computational graph}. As
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{
		images/16_DeepLearning_computationalGraphNeuralNetwork2.png
	}
	\caption{The forward propagation rule in GNNs. It determines how the information
	from the input will go to the output side of the neural network.}
	\label{fig:computationalGraphNeuralNetwork1}
\end{figure}

\subsubsection{Graph Convolutional Networks}
GCNs were first introduced in "Spectral Networks and Deep Locally Connected Networks
on Graphs" (Bruna et al, 2014), as a method for applying neural networks to
...
\end{itemize}
The operations are usually done in this order. Together, they make up one network
layer. We can combine one or more layers to form a complete GCN.

\subsubsection{Applications of GNNs}
Graph-structured data is present everywhere. The problems that GNNs resolve can be
